...
$p_name = mysqli_real_escape_string($condb,$_POST["p_name"]);
    ...
    $p_price = mysqli_real_escape_string($condb, $_POST["p_price"]);
    $p_qty = mysqli_real_escape_string($condb, $_POST["p_qty"]);
    $file1 = $_POST['file1']; // รับไฟล์รูปเดิมที่ใช้
    ...
    if ($upload != '') { // ถ้ามีการอัปโหลดรูปใหม่
        ...
        // ลบรูปภาพเก่าออกจากโฟลเดอร์ ถ้ามี
        if (!empty($file1) && file_exists($path . $file1)) {
            unlink($path . $file1);
        }
    } else {
        $newname = $file1; // ถ้าไม่มีการอัปโหลดรูปใหม่ ใช้รูปเก่า
    }
...
elseif (isset($_GET['product']) && $_GET['product'] == "del") { 
    ...
    if ($result) {
        ...
        echo "<script type='text/javascript'>";
        echo "window.location = 'list_product.php?product_del_error=product_del_error'; ";
        echo "</script>";
    }
